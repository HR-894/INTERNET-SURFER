...
async def increment_usage(user_id: str):
    ...
    month_url = f"{FIREBASE_DB_URL}/usage_images/{_month_key()}/total_count.json"
...
# ---------------- Cooldown ----------------
async def check_and_update_cooldown(user_id: str):
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    txt = (
        ...
        "• /search `<math or query>` — Safe math or Google\n"
        "• /image `<prompt>` [options] — AI image\n"
        "• /quota — daily usage\n"
        "Admin: /resetquota, /setlimit, /resetmonth, /checkquota, /stats"
    )
    await update.message.reply_markdown(txt)

# … (You can keep all other commands `/ask`, `/search`, `/image`, `/quota`, admin commands as in your original code)
...
async def post_init(apply):
    await apply.bot.set_my_commands([
        ...
        BotCommand("search", "Safe math/Google"),
